...
await ev("enableCardManualV2();MAG_STUDIO.cardEdits[1].imageIndex=2;MAG_STUDIO.cardEdits[1].photoMode='trio';MAG_STUDIO.cardEdits[1].preset='noir';MAG_STUDIO.cardEdits[1].textScale=1.2;MAG_STUDIO.cardEdits[1].textX=.08;MAG_STUDIO.cardEdits[1].textY=.06;renderCardNewsV2();true");await new Promise(r=>setTimeout(r,120));
out.cardControls={images:await ev("document.querySelectorAll('[data-card-image]').length"),modes:await ev("document.querySelectorAll('[data-card-mode]').length"),presets:await ev("document.querySelectorAll('[data-card-preset]').length"),scales:await ev("document.querySelectorAll('[data-card-scale]').length"),xs:await ev("document.querySelectorAll('[data-card-x]').length"),ys:await ev("document.querySelectorAll('[data-card-y]').length"),tune:await ev("cardTextTuneV2(MAG_STUDIO.cardEdits[1])"),thumbTrio:await ev("document.querySelectorAll('.cardnews-thumbs article:nth-child(2) .card-thumb-media img').length"),thumbPreset:await ev("document.querySelector('.cardnews-thumbs article:nth-child(2)')?.className")};
