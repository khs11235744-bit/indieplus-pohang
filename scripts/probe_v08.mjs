...
out.visualPaths=await ev("cardVisualsV2(studioItemFromForm())");out.selectedStills=await ev("studioItemFromForm().stillIndices");out.previewSecondaryImages=await ev("document.querySelectorAll('.mag-page-photo-strip img').length");out.cardThumbs=await ev("document.querySelectorAll('.cardnews-thumbs article').length");out.cardImages=await ev("[...document.querySelectorAll('.cardnews-thumbs img')].map(x=>({src:x.getAttribute('src'),loaded:x.complete&&x.naturalWidth>0}))");
