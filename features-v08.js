...
async function makeCardSlideBlobV2(item,slide,index){
  ...
  const titleBase=format==="story"?82:format==="square"?62:74,titleSize=Math.round(titleBase*tune.scale),titleGap=Math.round(titleSize*1.12);
  ...
  let y=start+86+textY+ts.length*titleGap+30;const bodyBase=format==="story"?50:format==="square"?36:44,bodySize=Math.round(bodyBase*tune.scale),bodyGap=Math.round(bodySize*1.42),maxLines=format==="story"?8:format==="square"?5:6;
...
function enableCardManualV2(){
  ...
  MAG_STUDIO.cardEdits=cardAutoSlidesV2(item).map((s,i)=>({...s,imageIndex:visuals.length?i%visuals.length:-1,photoMode:"single",preset:item.preset||"journal",textScale:1,textX:0,textY:0}));
