...
function currentCineTip(){
  ...
  return arr[((cineTipCursor%arr.length)+arr.length)%arr.length];
}
function randomCineTip(){
  const arr=CINE_TIPS?.tips||[];if(!arr.length)return;
  if(arr.length===1){cineTipCursor=0;return renderCineTip()}
  let next=cineTipCursor;
  while(next===cineTipCursor)next=Math.floor(Math.random()*arr.length);
  cineTipCursor=next;renderCineTip();
}
...
function injectCinephileLens(){
  ...
  sec.innerHTML='<article class="cine-tip-card"><div class="cine-tip-head"><div><div class="kicker">DAILY CINEPHILE LENS</div><span id="cineTipCategory" class="cine-tip-cat"></span></div><div class="cine-tip-count">100개의 보는 법</div></div>'+
    '<blockquote id="cineTipText">오늘의 랜덤 시네필 포인트를 고르는 중입니다.</blockquote>'+
    '<div class="cine-tip-mission"><b>볼 포인트</b><span id="cineTipMission"></span></div>'+
    '<div class="cine-tip-footer"><small id="cineTipSource"></small><div><button class="text-button" id="cineTipPrev">랜덤</button><button class="text-button" id="cineTipNext">또 다른 랜덤</button><button class="text-button" id="cineTipSave">☆ 저장</button><button class="text-button" id="cineTipShare">공유</button></div></div>'+
  ...
  document.getElementById("cineTipPrev").onclick=randomCineTip;
  document.getElementById("cineTipNext").onclick=randomCineTip;
...
function renderCineLibrary(cat){
  ...
  root.innerHTML=arr.map(t=>'<article><div><span>'+esc(t.category)+'</span><small>#'+t.id+'</small></div><b>'+esc(t.tip)+'</b><p>볼 포인트 · '+esc(t.mission)+'</p><button onclick="toggleCineTipSave('+t.id+');renderCineLibrary(\''+esc(cat)+'\')">'+(saved.includes(t.id)?"★":"☆")+'</button></article>').join("");
}
async function initV07(){
  ...
  if(!CINE_TIPS)return;cineTipCursor=Math.floor(Math.random()*Math.max(1,CINE_TIPS.tips?.length||1));injectCinephileLens();renderCineTip();
}
initV07();
